...
wm_only <- ML.df[!is.na(ML.df$FCH4_gC), ]

# 75% of data used for model tuning/validation
index <- createDataPartition(wm_only$FCH4_gC, p=0.75, list=F) 
...
# time series
result$datetime <- data.daily.model.norm$datetime
result$site <- data.daily.model.norm$site

result %>% ggplot(aes(datetime,FCH4,color = as.factor(site))) + geom_point() + 
...
# https://stackoverflow.com/questions/54521395/extract-plotting-data-from-partial-dependence-plot-built-from-random-forest
# https://www.nature.com/articles/s41598-023-30313-8
# https://blogs.ubc.ca/tdeenik/2021/07/22/random-forest-modelling-in-r-step-by-step-process/
# https://www.cambridge.org/core/journals/environmental-data-science/article/variable-ranking-and-selection-with-random-forest-for-unbalanced-data/D00D9D74FA395B4FAC8886A84CC2FCCA